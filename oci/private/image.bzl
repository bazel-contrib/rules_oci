...
"""),
    ...
    "labels": attr.label(doc = "A file containing a dictionary of labels. Each line should be in the form `name=value`.", allow_files = True),
    "annotations": attr.label(doc = "A file containing a dictionary of annotations. Each line should be in the form `name=value`.", allow_files = True),
...
def _oci_image_impl(ctx):
    ...
    if ctx.attr.labels:
        args.add_all(ctx.files.labels, format_each = "--labels=%s")
        inputs += ctx.files.labels

    if ctx.attr.annotations:
        args.add_all(ctx.files.annotations, format_each = "--annotations=%s")
