...
load("//oci/private:versions.bzl", "APKO_VERSIONS", "CRANE_VERSIONS", "ZOT_VERSIONS")
...
load("@rules_oci//oci:toolchain.bzl", "apko_toolchain")
...
apko_repositories = repository_rule(
    _apko_repo_impl,
    doc = "Fetch external tools needed for apko toolchain",
    attrs = {
        "apko_version": attr.string(mandatory = True, values = APKO_VERSIONS.keys()),
        "platform": attr.string(mandatory = True, values = PLATFORMS.keys()),
        "_launcher_tpl": attr.label(default = "//oci/private/registry:apko_launcher.sh.tpl"),
    },
)
...
def oci_register_toolchains(name, crane_version, zot_version = None, apko_version = LATEST_APKO_VERSION, register = True):
    ...
    Args:
        ...
        apko_version: passed to each apko_repositories call.
          NB: A default value is set to make addition of apko a non-breaking change to rules_oci
    ...
    apko_toolchain_name = "{name}_apko_toolchains".format(name = name)
    ...
    for platform in PLATFORMS.keys():
        apko_repositories(
            name = "{name}_apko_{platform}".format(name = name, platform = platform),
            platform = platform,
        ...
        if register:
            native.register_toolchains("@{}//:{}_toolchain".format(apko_toolchain_name, platform))
    ...
    toolchains_repo(
        name = apko_toolchain_name,
        toolchain_type = "@rules_oci//oci:apko_toolchain_type",
        # avoiding use of .format since {platform} is formatted by toolchains_repo for each platform.
        toolchain = "@%s_apko_{platform}//:apko_toolchain" % name,
    )
    toolchains_repo(
