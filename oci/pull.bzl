...
def _oci_pull_impl(rctx):
    ...
    manifest_file = _trim_hash_algorithm(rctx.attr.identifier)
    manifest, size = _download_manifest(rctx, rctx.attr.identifier, manifest_file)

    if _is_image(manifest):
        if rctx.attr.platform:
            fail("{} is a single-architecture image, so attribute 'platform' should not be set.".format(rctx.attr.image))
        image_manifest_file = manifest_file
        image_manifest = manifest
        image_manifest_size = size
        image_digest = _sha256(rctx, coreutils, image_manifest_file) if _is_tag(rctx.attr.identifier) else rctx.attr.identifier

    elif _is_image_index(manifest):
        ...
        image_digest = matching_manifest["digest"]
        image_manifest_file = _trim_hash_algorithm(image_digest)
        image_manifest, image_manifest_size = _download_manifest(rctx, image_digest, image_manifest_file)

    else:
        fail("Unrecognized mediaType {} in manifest file".format(manifest["mediaType"]))

    image_config_file = _trim_hash_algorithm(image_manifest["config"]["digest"])
    _download(rctx, image_manifest["config"]["digest"], image_config_file)
    image_config = json.decode(rctx.read(image_config_file))
    ...
    if _is_image_index(manifest):
        ...
        if "variant" in image_config:
            manifest_json["platform"]["variant"] = image_config["variant"]
    ...
    for layer in image_manifest["layers"]:
        digest = _trim_hash_algorithm(layer["digest"])

        # TODO: we should avoid eager-download of the layers ("shallow pull")
        _download(rctx, layer["digest"], digest)
        blobs.append(digest)
